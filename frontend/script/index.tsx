  ...
  for (const key in types) {
    ...
    switch (type) {
      ...
      case 'button': {
        ...
        const { shadowHost: buttonShadowHost, shadowRoot: buttonShadowRoot } =
          createShadowDOM()

        const font = getFontFamily(config.buttonFontName, 'button')
        buttonShadowHost.style.setProperty(
          '--wmt-button-font',
          font.selectedFont
        )
...
const setupButtonPlaceholders = (
  config: Config,
  buttonShadowRoot: ShadowRoot
) => {
  ...
  buttonPlaceholders.forEach((placeholder) => {
    ...
    button.addEventListener('click', (e) => {
      ...
      const overlay = buttonShadowRoot.querySelector(
        '._wm_tools_button_overlay'
      )
