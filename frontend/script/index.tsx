...
import { OpenPaymentsExtension } from './OpenPaymentsExtension';

window.Buffer = Buffer;
window.process = { ...window.process, env: { NODE_ENV: 'development',NODE_DEBUG: 'true' } };  // Adjust if needed
...
fetch(`${API_URL}tools/${urlWallet}`)
  ...
  .then((resp) => {
    const config = resp
    //console.log("rrrrresp ", paramTypes, paramWallet as string, config)
...
function renderOPComponent(
  wrapper: ReactDOM.Container,
  type: string,
  ...
  for (let key in types) {
    ...
    switch (type) {
      case "widget": {
        ...
        const cssbase = getCSSFile("css/script.css")
        const element = drawWidget(walletAddress, config)
        shadowRoot.appendChild(css)
        shadowRoot.appendChild(cssbase)
        shadowRoot.appendChild(element)
        document.body.appendChild(shadowHost)
        
        const wrapper = shadowRoot.querySelector('#_wm_tools_op_wrapper');
        renderOPComponent(wrapper as Element,type, walletAddress, config)
        break
      }
      case "banner":
      default:
...
const drawWidget = (walletAddress: string, config: any) => {
  ...
  content.innerHTML = `
        ...
        <div id="_wm_tools_op_wrapper" class="_wm_tools_widget_iframe_wrapper">       
        </div>`
